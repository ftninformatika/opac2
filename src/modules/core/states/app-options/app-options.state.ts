...
export class AppOptionsState {
  ...
  public static getShareSelectionRecords(state: IAppOptionsState): string[] {
    if (
      !state ||
      !state.selectedShareRecords ||
      state.selectedShareRecords.length === 0
    ) {
      return null;
    }
  ...
  @Selector()
  public static getSelectedLocationFilters(
    state: IAppOptionsState
  ): ISelectedFilter[] {
    if (
      !state ||
  ...
  @Selector()
  public static getSelectedSubLocationFilters(
    state: IAppOptionsState
  ): ISelectedFilter[] {
    if (
      !state ||
      !state.selectedSubLocations ||
      state.selectedSubLocations.length === 0
    ) {
      return [];
    }
  ...
  public addRemoveRecordIdToSelected(
  ...
  public addRemoveMultipleRecordIdToSelected(
    ctx: StateContext<IAppOptionsState>,
    action: AddMultipleIdsToSelectedAction
  ) {
    const state = ctx.getState();
  ...
  public addRemoveLocationsAction(
    ctx: StateContext<IAppOptionsState>,
    action: AddRemoveLocationsAction
  ) {
    const state = ctx.getState();
  ...
  public addRemoveSubLocationsAction(
    ctx: StateContext<IAppOptionsState>,
    action: AddRemoveSubLocationsAction
  ) {
    const state = ctx.getState();
