...
export class LocationComponent implements OnInit {
  ...
  onBtnSaveLocation() {
    if (this.libraryForm.invalid) {
      this.toastService.warning($localize`:@@popuniteObaveznaPolja:Попуните сва обавезна поља`);
      return;
    }
    ...
    if (!latitudeCheck || !longitudeCheck) {
      this.toastService.warning($localize`:@@longitudaLatitudaFormat:Лонгитуда или латитуда нису унете у исправном формату`);
      return;
    }
    if (this.editing) {
      this.edit();
  ...
  add() {
    ...
    this.libraryService.create(this.library).subscribe(savedLibrary => {
      if (savedLibrary) {
        ...
        this.toastService.success($localize`:@@uspesnoDodataBiblioteka:Успешно сте додали нову библиотеку`);
      } else {
        this.toastService.error($localize`:@@greskaDodavanjeBiblioteke:Дошло је до грешке приликом додавања нове библиотеке. Покушајте поново`);
      }
    }, () => {
      this.toastService.error($localize`:@@greskaDodavanjeBiblioteke:Дошло је до грешке приликом додавања нове библиотеке. Покушајте поново`);
    });
  ...
  edit() {
    this.libraryService.edit(this.library).subscribe(editedLibrary => {
      if (editedLibrary) {
        ...
        this.toastService.success($localize`:@@uspesnoIzmenjenaBiblioteka:Успешно сте изменили библиотеку`);
        this.closeDialog();
      } else {
        this.toastService.error($localize`:@@greskaIzmenaBiblioteke:Дошло је до грешке приликом измене библиотеке. Покушајте поново`);
      }
    }, () => {
      this.toastService.error($localize`:@@greskaIzmenaBiblioteke:Дошло је до грешке приликом измене библиотеке. Покушајте поново`);
    });
  ...
  delete() {
    this.libraryService.delete(this.libraryToDelete._id).subscribe(response => {
      if (response) {
        ...
        this.toastService.success($localize`:@@uspesnoObrisanaBiblioteka:Успешно сте обрисали библиотеку`);
      } else {
        this.toastService.error($localize`:@@greskaBrisanjeBiblioteke:Дошло је до грешке приликом брисања библиотеке. Покушајте поново`);
      }
    }, () => {
      this.toastService.error($localize`:@@greskaBrisanjeBiblioteke:Дошло је до грешке приликом брисања библиотеке. Покушајте поново`);
    });
